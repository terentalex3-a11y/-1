...
func _input(event):
    ...
    if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
        ...
        if Rect2(1040, 15, 210, 45).has_point(p):
            open_inventory()
            return
        if combat:
...
func _draw():
    ...
    draw_button(Rect2(1040, 15, 210, 45), "Инвентарь")
    var wound_text := "Ранения: голова %d | корпус %d | рука %d | нога %d" % [wounds.head, wounds.torso, wounds.arm, wounds.leg]
    draw_string(ThemeDB.fallback_font, Vector2(25, 620), wound_text, HORIZONTAL_ALIGNMENT_LEFT, 800, 15, Color("c99b9b"))
    draw_string(ThemeDB.fallback_font, Vector2(25, 650), message, HORIZONTAL_ALIGNMENT_LEFT, 1000, 17, Color("eeeeee"))
    if combat:
