...
static func apply_mod(weapon: Dictionary, mod: Dictionary) -> Dictionary:
    var result = weapon.duplicate(true)
    var mods: Array = result.get("mods",[])
    mods.append(mod.duplicate(true))
    result["mods"] = mods
    return result

static func weapon_accuracy(weapon: Dictionary) -> int:
    var value := 0
    for mod in weapon.get("mods",[]):
        value += int(mod.get("accuracy",0))
    return value

static func weapon_damage_bonus(weapon: Dictionary) -> int:
    var value := 0
    for mod in weapon.get("mods",[]):
        value += int(mod.get("damage",0))
    return value
